...
- Commit compared: `f9a537c` (2025-03-02) — this **is** `origin/master` as of the writing/fixing
  of this backlog (verified: 0 commits between them). The `v0.11` tag is 18 commits behind master
  and is missing the `fast`-mode flag and STM32L4/Black-Magic-Probe pipe-stall workarounds that
  `dfuse.c`/`dfuse.py` already assume — so all verification against upstream in this document
  targets **master**, not `v0.11`.
...
25. ✅ **DONE** — **`__main__.py:559-571`** — `-R`/`--reset` no longer called
    `dfu_root.detach(1000)` before `dev.reset()` — the call itself was dropped even though the
    surrounding "Even if detach failed..." comment survived. Some devices need the explicit
    DFU_DETACH before a clean bus reset back to runtime.
    C ref: `main.c:773-780`.
    Fix: added `try: ret = dfu_root.detach(1000) except USBError: ret = -1` right before the
    `int_(ret) < 0` check, matching the same try/except pattern already used for `Mode.DETACH`.

26. ✅ **DONE** — **`__main__.py:511`** — `open(file.name, "wb")` for upload silently overwrote an
    existing output file. C opens with `O_WRONLY|O_CREAT|O_EXCL|O_TRUNC` — fails instead of
    clobbering.
    C ref: `main.c:719`.
    Fix: `fd = os.open(file.name, os.O_WRONLY | os.O_CREAT | os.O_EXCL | os.O_TRUNC)` then
    `os.fdopen(fd, "wb")` as `file.file_p`; `FileExistsError` (a subclass of `IOError`/`OSError`)
    is already caught by the existing `except IOError` handler. Verified live: attempting to open
    a pre-existing path now raises `FileExistsError` instead of truncating it.

27. ✅ **DONE** — **`__main__.py:727-737`** (the `else` branch when the runtime-vs-DFU-mode device
    is "already in DFU mode?") — C does `goto dfustate`, which **skips** the following
    release/close/`disconnect_devices`/`Mode.DETACH`-exit/`milli_sleep(detach_delay)`/re-probe
    block entirely. Python called `dfu_state()` directly here but then **fell through** to that
    same block anyway (textually — in practice `dfu_state()` never returns because its own body
    ends in `sys.exit()` on every path, so this was latent/dead-code-shaped rather than an
    observed double-run, but still relied on that fragile side effect instead of being structurally
    correct), and called `dfu_state()` a **second time** afterward.
    C ref: `main.c:555-561` (`goto dfustate`).
    Fix: added an explicit `return` right after the `dfu_state()` call in this branch, so the
    control flow structurally mirrors C's `goto dfustate` instead of depending on `dfu_state()`
    always exiting the process.

28. ✅ **DONE** — **`__main__.py:589-595`** (`probe()`, "no DFU capable USB device available") —
    did `return SysExit.EX_IOERR`, but `probe()` is a nested function whose return value was never
    used by the caller (`__main__.py:802` just calls `probe()` as a bare statement); the process
    exited with code `0` instead of the intended nonzero error code. Distinct from the already-
    ...
    in this function. Verified with a mocked empty-device-list run: exit code is now `74`
    (`EX_IOERR`) instead of `0`.

29. ✅ **DONE** — **`__main__.py:414,422`** (`check_status()`) — recursed on error/abort
    (`check_status()`) but discarded the return value, then fell through to `return status` (the
    stale, pre-recheck object) instead of the freshly-fetched one. C's `goto status_again`
    naturally continues with the fresh status.
    Fix: `return check_status()` in both recursive branches.

### `dfu.py`

30. ✅ **DONE** — **QUIRK_POLLTIMEOUT was only applied at one call site instead of centrally.** C
    applies the override inside `dfu_get_status()` itself (`dfu.c:156-161`), so every caller
    benefits automatically. Python's `_get_status`/`StatusRetVal.from_bytes` (`dfu.py:367-397`,
    `129-147`) never received `quirks` at all; the override was instead re-implemented ad hoc only
    in `dfu_load.py`'s poll loop. **Not applied** in `dfu.py::_abort_to_idle` (line 554, raw
    `dst.bwPollTimeout`) nor in `dfuse.py` (lines 267, 361, also raw).
    Fix: applied centrally in `DfuIf.get_status()` (`dfu.py:223-229`) — after calling
    `_get_status()`, override `status.bwPollTimeout = DEFAULT_POLLTIMEOUT` if
    `self.quirks & QUIRK.POLLTIMEOUT`. Since `_abort_to_idle`, `dfuse.py`'s `special_command()`/
    `download_chunk()`, and `dfu_load.py` all fetch status exclusively through `dif.get_status()`,
    they all became quirk-aware automatically with this one change — no per-call-site guards
    needed anymore (see #31/#32, fixed as part of the same change). Verified live with a mocked
    6-byte GETSTATUS response: with `quirks=QUIRK.POLLTIMEOUT` set, `bwPollTimeout` comes back as
    `DEFAULT_POLLTIMEOUT` (5) instead of the device-reported value; without the quirk, the raw
    device value passes through unchanged.

### `dfu_load.py`

31. ✅ **DONE** — **`dfu_load.py:188-189`** —
    `if not (dif.quirks and dif.quirks & QUIRK.POLLTIMEOUT): milli_sleep(status.bwPollTimeout)`
    — skipped the sleep entirely when the quirk was set, instead of sleeping `DEFAULT_POLLTIMEOUT`.
    Inconsistent with the correct pattern used two lines earlier at `dfu_load.py:156-160`.
    Fix: superseded by #30 — now that `dif.get_status()` returns an already quirk-adjusted
    `bwPollTimeout`, both poll sites just do a plain, unconditional `milli_sleep(status.bwPollTimeout)`.

32. ✅ **DONE** — **`dfu_load.py:192-200`** — the manifest-wait polling loop only slept a fixed
    `1000`ms per iteration; C also sleeps `bwPollTimeout` (quirk-aware) on every re-poll, right
    after each `dfu_get_status()`/`goto get_status`, in addition to the extra 1000ms before looping
    back. Poll-timeout math diverged from C on every manifest re-poll.
    C ref: `dfu_load.c:192-200` (`milli_sleep(dst.bwPollTimeout)` unconditionally after every
    status fetch; `case dfuMANIFEST_SYNC/dfuMANIFEST: milli_sleep(1000); goto get_status;`).
    Fix: restructured the loop so `milli_sleep(status.bwPollTimeout)` runs after *every*
    `dif.get_status()` call (the initial "Transition to MANIFEST_SYNC" fetch and each loop
    iteration's re-fetch), with the extra `milli_sleep(1000)` only between re-fetches — matching
    C's do-while/goto structure exactly.
